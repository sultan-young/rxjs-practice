...
import { concat, interval, map, merge, mergeAll, mergeMap, Observable, of, race, take } from "rxjs";
...
const mergeStream = interval(1000).pipe(take(2), map(i => [stream1, stream2][i]));
 ...
 */
const mergeMapStream = interval(1000).pipe(take(2), mergeMap(i => [stream1, stream2][i]))
